...
def pdfReader(pdf_path):
    ...
    with open(pdf_path, 'rb') as pdfFileObject:
        ...
        # Prompt the user for reading the entire PDF or a specific range
        c = input("Do you want to read the entire PDF? [Y]/N: ").strip()
        start_page = 0
        end_page = count - 1

        if c.lower() == 'n':
            # Get start and end page numbers from the user
            start_page = int(input("Enter start page number (Index starts from 0): "))
